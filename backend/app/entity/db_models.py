...
from sqlalchemy import Column, Integer, String, Float, DateTime, ForeignKey, JSON, Text, Boolean, BigInteger
...
class Role(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    name = Column(String(50), unique=True, nullable=False, comment="角色标识,如admin/operator/viewer")
    display_name = Column(String(100), nullable=False, comment="角色显示名,如管理员/操作员/访客")
...
class DetectionScene(Base):
    ...
    display_name = Column(String(100), nullable=False, comment="场景显示名, 如遥感目标检测")
...
class TrainingTask(Base):
    ...
    task_uid = Column(String(100), unique=True, nullable=False, index=True, comment="任务唯一标识")
...
class ModelVersion(Base):
    ...
    precision = Column(Float, nullable=True, comment="精确率")
    recall = Column(Float, nullable=True, comment="召回率")
    per_class_ap = Column(JSON, nullable=True, comment="各类别AP，如 {\"airplane\":0.85,\"tank\":0.72}")
...
class ChatSession(Base):
    ...
    session_uid = Column(String(100), unique=True, nullable=False, index=True, comment="会话唯一标识")
